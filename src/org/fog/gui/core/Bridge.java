			...
			try {
				...
				while (iter.hasNext()) {
					...
					if (nodeType.equalsIgnoreCase("host")) {  // host
						...
						for (int n = 0; n < num; n++) {
							Node hNode = new HostNode(nodeName, nodeType, pes, mips,
								ram, storage, bw);
							graph.addNode(hNode);
						}
						...
						double rate = new BigDecimal((Double) node.get("ratePerMips"))
							.doubleValue();

						Node fogDevice = new FogDeviceGui(nodeName, mips, ram,
						...
						int downports = new BigDecimal((Long) node.get("downports"))
							.intValueExact();
