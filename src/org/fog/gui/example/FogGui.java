...
public class FogGui extends JFrame {
	...
	private void doPosition() {
		...
		// One could use the dimension of the frame. But when doing so, one have
		// to call this method !BETWEEN! pack and
	...
	private final void initBar() {
			...
			public void actionPerformed(ActionEvent event) {
				if ("i" == mode) {
					...
					if (deploymentFile == null || deploymentFile.isEmpty()) {
						JOptionPane.showMessageDialog(panel, "Please select deploymentFile",
							"Error", JOptionPane.ERROR_MESSAGE);
						return;
					}
					if (workloads_background == null || workloads_background.isEmpty()) {
						JOptionPane.showMessageDialog(panel, "Please select workloads_background",
							"Error", JOptionPane.ERROR_MESSAGE);
						return;
					}
					if (workloads == null || workloads.isEmpty()) {
						JOptionPane.showMessageDialog(panel, "Please select workloads", "Error",
		...
		// ImageIcon(getClass().getResource("/src/new.png"));
		...
		// switch mode between manual mode (to create graph by hand) and import
